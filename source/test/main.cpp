...
#include "../driver/account.h"   // To get the account class
...
void HTTest::insert_accounts(void) {
    ...
    for (auto &e : m_accounts) ht_accounts.insert(e.getKey(), e);
}
...
TEST_F(HTTest, OperatorSquareBraketsRHS) {
    ...
    for (auto &e : m_accounts) ASSERT_EQ(ht_accounts[e.getKey()], e);
}
...
TEST_F(HTTest, AtRHS) {
    ...
    // Retrieve each element
    for (auto &e : m_accounts) ASSERT_EQ(ht_accounts.at(e.getKey()), e);
}
...
TEST_F(HTTest, CopyConstructor) {
    ...
    ac::HashTbl<std::string, size_t> word_map;
    for (const auto &e : expected) word_map.insert(e.first, e.second);
...
TEST_F(HTTest, Count) {
    ...
    std::map<int, std::string> set1{
        {11, "eleven"}, {2 * 11, "twenty two"}, {3 * 11, "thirty three"}, {4 * 11, "fourty four"}};
    ...
    for (const auto &e : set1) ASSERT_EQ(set1.size(), htable.count(e.first));
    for (const auto &e : set2) ASSERT_EQ(set2.size(), htable.count(e.first));
    for (const auto &e : set3) ASSERT_EQ(set3.size(), htable.count(e.first));
    // std::cout << "The table: \n" << htable << std::endl;
}
